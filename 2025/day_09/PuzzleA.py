# Standard library
from pathlib import Path
from typing import Any

INPUT_FILE_PATH = Path(__file__).parent


def read_data(
    file_path: Path,
) -> list[str]:
    with open(file_path) as file:
        lines = [line.strip() for line in file]
    return lines


def parse_input(file_content: list[str]) -> Any:
    return


def solve_01(data: Any) -> int:
    # take first point and calculate rest of diagonals to other points
    # sort highest to lowest diagonals
    #     side a = abs(x1-x2)+1
    #     side b = abs(y1-y2)+1
    #     diagonal = math.sqrt(a**2+b**2)

    # Calculate real area highest diagonal
    #     A = a*b
    # Check only rest of diagonals that d >= math.sqrt(2*A)  # maximum area diagonal have is the square
    # Find largest rectange

    # Repeat for the other combinations of points
    # Directly consider only diagonals that d >= math.sqrt(2*A)

    return


def main() -> None:
    # input.txt | example_1.txt
    file_content = read_data(INPUT_FILE_PATH / "example_1.txt")
    data = parse_input(file_content)
    solution = solve_01(data)
    print(f"The solution of the example 1 is {solution}")  # Solution 50
    file_content = read_data(INPUT_FILE_PATH / "input.txt")
    data = parse_input(file_content)
    solution = solve_01(data)
    print(f"The solution of the part 1 is {solution}")


if __name__ == "__main__":
    main()
