...
def organize_rocks_by_columns(
    round_rocks: list[coord], cube_rocks: list[coord], max_col: int
...
def tilt_north_column(
    rows_round_rocks: list[int], rows_cube_rocks: [list[int]], max_col: int
) -> list[int]:
...
def solve_01(data) -> int:
    ...
    for col, rocks in column_platform.items():
        ...
        new_column_platform.update(
            {col: {"round": new_round_rocks, "cube": rocks["cube"]}}
        )
